function ImportResults(path)#### Set the path to saved data
    ### Import saved scoring data (NEBD, Start Congression, Anaphase Onset)
    scoring_df = CSV.read(joinpath(path, "scoring_df.csv"), DataFrame)

    ### Import saved Interphase data
    Interphase_df = DataFrame(CSV.File(joinpath(path, "InterphaseDF.csv"), delim=","))
